---
meta:
  title: 'Announcing Blotout EdgeTag: Fixing your Facebook Pixels in 2 min | Blotout'
  description: 'Latest news and insights from Blotout—the customer data platform reimagined for the trusted enterprise.'
  og:type: 'article'
  og:article:published_time: '2021-12-03T00:00:00.000Z'
data:
  date: '2021-12-03'
---
...
We’re proud to share Blotout <a href='https://edgetag.io/facebook-conversion-api' target='_blank' rel='noopener' onClick={() => codifyClick('EdgeTag Announcement Blog - EdgeTag Website')}>EdgeTag</a>, the first privacy-preserving fix for lossy and non-consented Facebook data sharing.
...
To take it for a spin, head to <a href='https://edgetag.io/facebook-conversion-api' target='_blank' rel='noopener' onClick={() => codifyClick('EdgeTag Announcement Blog - EdgeTag Website')}>edgetag.io/facebook-conversion-api</a> and get set up in only 2 minutes. Our Cloudflare [app](cloudflare.com/apps) for the same will be released by Dec 6, 2021.

## The journey to EdgeTag

Trust matters to customer data like never before. EdgeTag is one more way that Blotout is building infrastructure that companies can use to run their modern customer data stack with trust at the core. Here is how we got here.
...
As browsers and app store economies push back on data collection directly from sites and apps for marketing signals or 3P measurements, large players like Google and Facebook have acknowledged that companies need to handle user data and consent themselves before engaging third-party tools.
...
Today, we are announcing the support for a consented Facebook Conversion API that operates as a single-tenant at the CDN edge.

### How does it work?

Like any other javascript tag. The primary difference is that the tag runs in a company’s infrastructure and by default in Consent Mode. Companies use their CDN infrastructure to run Blotout software to manage its data.
...
By default EdgeTag requires customers to check for consent on the tag itself (customers can choose their own policies for jurisdictions where consent is not mandated). We are actively implementing the Global Privacy Control (GPC) for the upcoming release to make it compatible with California’s privacy law that is coming into effect in 2022.
...
We will be launching more microservices for marketing data activation, including Google Conversion.
